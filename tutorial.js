...
if (name == null) name = nil;
    ...
    def.$inspect = function() {
      ...
      result = $rb_plus(result, ($a = ($b = self.$each_pair()).$map, $a.$$p = (TMP_14 = function(name, value){var self = TMP_14.$$s || this;
...
Opal.modules["opal-jquery/http"] = function(Opal) {
  ...
  Opal.add_stubs(['$require', '$to_n', '$new', '$attr_reader', '$build_request', '$proc', '$ok?', '$resolve', '$reject', '$send!', '$delete', '$from_object', '$succeed', '$fail', '$call', '$parse', '$xhr']);
  self.$require("json");
  self.$require("native");
  ...
  return (function($base, $super) {
    ...
    Opal.defs(self, '$get', TMP_1 = function(url, opts) {
      ...
      TMP_1.$$p = null;
      return self.$build_request(url, "GET", opts, block);
    });
    ...
    Opal.defs(self, '$post', TMP_2 = function(url, opts) {
      ...
      TMP_2.$$p = null;
      return self.$build_request(url, "POST", opts, block);
    });
    ...
    Opal.defs(self, '$put', TMP_3 = function(url, opts) {
      ...
      TMP_3.$$p = null;
      return self.$build_request(url, "PUT", opts, block);
    });
    ...
    Opal.defs(self, '$delete', TMP_4 = function(url, opts) {
      ...
      TMP_4.$$p = null;
      return self.$build_request(url, "DELETE", opts, block);
    });
    ...
    Opal.defs(self, '$patch', TMP_5 = function(url, opts) {
      ...
      TMP_5.$$p = null;
      return self.$build_request(url, "PATCH", opts, block);
    });
    ...
    Opal.defs(self, '$head', TMP_6 = function(url, opts) {
      ...
      TMP_6.$$p = null;
      return self.$build_request(url, "HEAD", opts, block);
    });

    Opal.defs(self, '$build_request', function(url, method, options, block) {
      var $a, $b, TMP_7, self = this, promise = nil, http = nil;

      if (block !== false && block !== nil) {
        } else {
        promise = Opal.get('Promise').$new();
        block = ($a = ($b = self).$proc, $a.$$p = (TMP_7 = function(response){var self = TMP_7.$$s || this, $a;
if (response == null) response = nil;
      ...
      http = self.$new(url, method, options, block)['$send!']();
      return ((($a = promise) !== false && $a !== nil) ? $a : http);
    });
    ...
    def.$initialize = function(url, method, options, handler) {
      ...
      settings.success = function(data, status, xhr) {
        ...
        if (typeof(data) === 'object') {
          http.json = $scope.get('JSON').$from_object(data);
        }
    ...
    def.$fail = function() {
      ...
      self.ok = false;
      if ((($a = self.errback) !== nil && (!$a.$$is_boolean || $a == true))) {
...
Opal.modules["dare/window"] = function(Opal) {
  ...
  Opal.add_stubs(['$attr_accessor', '$attr_reader', '$[]', '$[]=', '$new', '$default_canvas', '$default_canvas=', '$add_mouse_event_listener', '$add_keyboard_event_listeners', '$update', '$draw', '$on', '$get_cursor_position', '$x', '$find', '$id', '$get_key_id', '$fill', '$page_x', '$page_y', '$Document', '$scrollLeft', '$documentElement', '$scrollTop', '$body', '$canvas', '$context']);
  return (function($base) {
    var self = $module($base, 'Dare');
...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$attr_reader', '$ms', '$rand', '$[]', '$%', '$to_i', '$size', '$draw_rot', '$new', '$offset_x', '$offset_y', '$reject!', '$distance', '$x', '$y', '$play', '$caption=', '$warp', '$load_tiles', '$draw', '$each', '$to_proc', '$score', '$button_down?', '$turn_left', '$turn_right', '$accelerate', '$move', '$collect_stars', '$<<', '$run!']);
  self.$require("dare");
  (function($base, $super) {
    function $Star(){};
    var self = $Star = $klass($base, $super, 'Star', $Star);

    var def = self.$$proto, $scope = self.$$scope;

    def.animation = def.creation_time = def.x = def.y = nil;
    self.$attr_reader("x", "y");

    def.$initialize = function(animation) {
      var self = this;

      self.animation = animation;
      self.creation_time = $scope.get('Dare').$ms();
      self.x = self.$rand(640);
      return self.y = self.$rand(480);
    };

    def.$draw = function() {
      var self = this, img = nil;

      img = self.animation['$[]'](($rb_divide(($rb_minus($scope.get('Dare').$ms(), self.creation_time)), 100)).$to_i()['$%'](self.animation.$size()));
      return img.$draw_rot(self.x, self.y);
    };
  ...
  (function($base, $super) {
    ...
    def.angle = def.vel_x = def.vel_y = def.x = def.y = def.image = def.score = def.grab_star_beep = nil;
    def.$initialize = function() {
      var self = this;

      self.image = (($scope.get('Dare')).$$scope.get('Image')).$new("assets/Starfighter.png");
      self.grab_star_beep = (($scope.get('Dare')).$$scope.get('Sound')).$new("assets/Beep.wav", $hash2(["volume", "overlap"], {"volume": 0.5, "overlap": 10}));
      self.x = self.y = self.vel_x = self.vel_y = 0.0;
      self.angle = 90.0;
      return self.score = 0;
    };

    def.$warp = function(x, y) {
      var $a, self = this;

      return $a = [x, y], self.x = $a[0], self.y = $a[1];
    };

    def.$turn_left = function() {
      var self = this;

      return self.angle = $rb_plus(self.angle, 4.5);
    };

    def.$turn_right = function() {
      var self = this;

      return self.angle = $rb_minus(self.angle, 4.5);
    };

    def.$accelerate = function() {
      var self = this;

      self.vel_x = $rb_plus(self.vel_x, $scope.get('Dare').$offset_x(self.angle, 0.5));
      return self.vel_y = $rb_plus(self.vel_y, $scope.get('Dare').$offset_y(self.angle, 0.5));
    };

    def.$move = function() {
      var self = this;

      self.x = $rb_plus(self.x, self.vel_x);
      self.y = $rb_plus(self.y, self.vel_y);
      self.x = self.x['$%'](640);
      self.y = self.y['$%'](480);
      self.vel_x = $rb_times(self.vel_x, 0.95);
      return self.vel_y = $rb_times(self.vel_y, 0.95);
    };

    def.$draw = function() {
      var self = this;

      return self.image.$draw_rot(self.x, self.y, self.angle);
    };
    ...
    return (def.$collect_stars = function(stars) {
      var $a, $b, $c, TMP_1, self = this;

      if ((($a = ($b = ($c = stars)['$reject!'], $b.$$p = (TMP_1 = function(star){var self = TMP_1.$$s || this;
        if (self.x == null) self.x = nil;
        if (self.y == null) self.y = nil;
if (star == null) star = nil;
      return $rb_lt($scope.get('Dare').$distance(self.x, self.y, star.$x(), star.$y()), 35)}, TMP_1.$$s = self, TMP_1), $b).call($c)) !== nil && (!$a.$$is_boolean || $a == true))) {
        self.score = $rb_plus(self.score, 1);
        return self.grab_star_beep.$play();
        } else {
        return nil
      };
    }, nil) && 'collect_stars';
  ...
  (function($base, $super) {
    ...
    def.player = def.background_image = def.stars = def.font = def.star_animation = nil;
    def.$initialize = TMP_2 = function() {
      var $a, $b, self = this, $iter = TMP_2.$$p, $yield = $iter || nil;
      ...
      (($a = ["Dare Tutorial Game"]), $b = self, $b['$caption='].apply($b, $a), $a[$a.length-1]);
      self.background_image = (($scope.get('Dare')).$$scope.get('Image')).$new("assets/Space.png");
      self.player = $scope.get('Player').$new();
      self.player.$warp(320, 240);
      self.star_animation = (($scope.get('Dare')).$$scope.get('Image')).$load_tiles("assets/Star.png", $hash2(["width"], {"width": 25}));
      self.stars = [];
      return self.font = (($scope.get('Dare')).$$scope.get('Font')).$new($hash2(["font", "size", "color"], {"font": "Arial", "size": 20, "color": "yellow"}));
    };
    ...
    def.$draw = function() {
      ...
      self.player.$draw();
      ($a = ($b = self.stars).$each, $a.$$p = "draw".$to_proc(), $a).call($b);
      return self.font.$draw("Score: " + (self.player.$score()), 20, 20);
    };
    ...
    return (def.$update = function() {
      ...
      ($a = ($b = self.stars).$each, $a.$$p = "update".$to_proc(), $a).call($b);
      self.player.$move();
      self.player.$collect_stars(self.stars);
      if ((($a = (($c = $rb_lt(self.$rand(100), 4)) ? $rb_lt(self.stars.$size(), 25) : $c)) !== nil && (!$a.$$is_boolean || $a == true))) {
        return self.stars['$<<']($scope.get('Star').$new(self.star_animation))
        } else {
        return nil
      };
    }, nil) && 'update';
  })(self, (($scope.get('Dare')).$$scope.get('Window')));
  return $scope.get('Game').$new()['$run!']();
})(Opal);
